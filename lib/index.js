...
app.use('/docs', express.static('jeve'));
app.set('etag', false);
...
class Jeve {
  ...
  #JSON_() {
    ...
    if (!fs.existsSync('jeve')) {
      fs.mkdirSync('jeve');
    }
    if (!fs.existsSync('jeve/api.json')) {
      this.#writeJSON(data);
    } else {
      const fileData = fs.readFileSync('jeve/api.json', { encoding: 'utf-8' });
      if (fileData !== data) {
        this.#writeJSON(data);
      }
    }
  ...
  #writeJSON(data) {
    fs.writeFileSync('jeve/api.json', data, 'utf-8');
  }
